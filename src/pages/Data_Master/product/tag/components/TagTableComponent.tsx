...
import { deleteTag, getAllTag, getOneTag, updateStatus } from "../store/tag.asyncAction";
import { tagAction } from "../store/tag.slice";

const TagTableComponent = () => {

	const dispatch = useDispatch<any>();
	const { getDataLoading, data, meta, showModalDelete, dataSelected, isRefresh } = useSelector((state: any) => state?.masterState?.ProductTag);
	...
	const onDelete = () => {
		dispatch(deleteTag(dataSelected));
	}
			...
			cell: ({ row }: any) => {
				...
				return (
					<div className="flex gap-5 justify-center">
						...
						<button
							onClick={() => {
								dispatch(tagAction.setShowModalDelete(true))
								dispatch(tagAction.setDataSelected(id))
							}}
							type="button"
	...
	return (
		<React.Fragment>
			<DeleteModal show={showModalDelete} onHide={() => dispatch(tagAction.setShowModalDelete(false))} onDelete={() => onDelete()} />
