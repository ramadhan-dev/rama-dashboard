...
const AddBrandComponents = lazy(() => import("./components/AddBrandComponents"));
const BrandTableComponent = lazy(() => import("./components/BrandTableComponent"));
...
const ProductBrandComponent = () => {
	...
	const { pageTitle } = useSelector((state: any) => state?.masterState?.ProductBrand);
				...
				<div className="card-body">
					<div className="flex items-center gap-3 mb-4">
						...
						<div className="shrink-0">
							<button data-modal-target="addCityModal" className="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20 add-employee" onClick={() => dispatch(brandAction.setOpenModalAdd(true))}>
								<Plus className="inline-block size-4" /> <span className="align-middle">Add Brand</span>
							</button>
						</div>
					</div>
