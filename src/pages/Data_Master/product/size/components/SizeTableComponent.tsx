...
import { deleteSize, getAllSize, getOneSize, updateStatus } from "../store/size.asyncAction";
import { sizeAction } from "../store/size.slice";

const SizeTableComponent = () => {

	const dispatch = useDispatch<any>();
	const { getDataLoading, data, meta, showModalDelete, dataSelected, isRefresh, buttonDisable } = useSelector((state: any) => state?.masterState?.ProductSize);
	...
	const editData = (id: string) => {
		dispatch(getOneSize(id));
	}

	const onDelete = () => {
		dispatch(deleteSize(dataSelected));
	}
			...
			cell: ({ row }: any) => {
				...
				return (
					<div className="flex gap-5 justify-center">
						...
						<button
							onClick={() => {
								dispatch(sizeAction.setShowModalDelete(true))
								dispatch(sizeAction.setDataSelected(id))
							}}
							type="button"
	...
	return (
		<React.Fragment>
			<DeleteModal show={showModalDelete} onHide={() => dispatch(sizeAction.setShowModalDelete(false))} onDelete={() => onDelete()} />
