...
const AddCategoryComponents = lazy(() => import("./components/AddCategoryComponents"));
const CategoryTableComponent = lazy(() => import("./components/CategoryTableComponent"));
...
const ProductCategoryComponent = () => {
	...
	const { pageTitle } = useSelector((state: any) => state?.masterState?.ProductCategory);
				...
				<div className="card-body">
					<div className="flex items-center gap-3 mb-4">
						...
						<div className="shrink-0">
							<button data-modal-target="addCityModal" className="text-white btn bg-custom-500 border-custom-500 hover:text-white hover:bg-custom-600 hover:border-custom-600 focus:text-white focus:bg-custom-600 focus:border-custom-600 focus:ring focus:ring-custom-100 active:text-white active:bg-custom-600 active:border-custom-600 active:ring active:ring-custom-100 dark:ring-custom-400/20 add-employee" onClick={() => dispatch(categoryAction.setOpenModalAdd(true))}>
								<Plus className="inline-block size-4" /> <span className="align-middle">Add Category</span>
							</button>
						</div>
					</div>
