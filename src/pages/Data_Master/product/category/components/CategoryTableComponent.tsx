...
import { deleteCategory, getAllCategory, getOneCategory, updateStatus } from "../store/category.asyncAction";
import { categoryAction } from "../store/category.slice";

const CategoryTableComponent = () => {

	const dispatch = useDispatch<any>();
	const { getDataLoading, data, meta, showModalDelete, dataSelected, isRefresh, buttonDisable } = useSelector((state: any) => state?.masterState?.ProductCategory);
	...
	useEffect(() => {
		...
		dispatch(getAllCategory(resetMeta));
	...
	useEffect(() => {
		if (isRefresh) {
			...
			dispatch(getAllCategory(newMeta));
		}
	}, [isRefresh])
	...
	const onDelete = () => {
		dispatch(deleteCategory(dataSelected));
	}
			...
			cell: ({ row }: any) => {
				...
				return (
					<div className="flex gap-5 justify-center">
						...
						<button
							onClick={() => {
								dispatch(categoryAction.setShowModalDelete(true))
								dispatch(categoryAction.setDataSelected(id))
							}}
							type="button"
	...
	return (
		<React.Fragment>
			<DeleteModal show={showModalDelete} onHide={() => dispatch(categoryAction.setShowModalDelete(false))} onDelete={() => onDelete()} />
