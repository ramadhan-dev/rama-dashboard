...
import { deleteVisibility, getAllVisibility, getOneVisibility, updateStatus } from "../store/visibility.asyncAction";
import { visibilityAction } from "../store/visibility.slice";

const VisibilityTableComponent = () => {

	const dispatch = useDispatch<any>();
	const { getDataLoading, data, meta, showModalDelete, dataSelected, isRefresh } = useSelector((state: any) => state?.masterState?.ProductVisibility);
	...
	useEffect(() => {
		...
		dispatch(getAllVisibility(resetMeta));
	...
	useEffect(() => {
		if (isRefresh) {
			...
			dispatch(getAllVisibility(newMeta));
		}
	}, [isRefresh])
	...
	const editData = (id: string) => {
		dispatch(getOneVisibility(id));
	}

	const onDelete = () => {
		dispatch(deleteVisibility(dataSelected));
	}
	...
	return (
		<React.Fragment>
			<DeleteModal show={showModalDelete} onHide={() => dispatch(visibilityAction.setShowModalDelete(false))} onDelete={() => onDelete()} />
...
export default VisibilityTableComponent
