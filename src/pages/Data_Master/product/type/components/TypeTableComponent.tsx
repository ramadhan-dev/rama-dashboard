...
import { deleteType, getAllType, getOneType, updateStatus } from "../store/type.asyncAction";
import { typeAction } from "../store/type.slice";

const TypeTableComponent = () => {

	const dispatch = useDispatch<any>();
	const { getDataLoading, data, meta, showModalDelete, dataSelected, isRefresh } = useSelector((state: any) => state?.masterState?.ProductType);
	...
	const onDelete = () => {
		dispatch(deleteType(dataSelected));
	}
			...
			cell: ({ row }: any) => {
				...
				return (
					<div className="flex gap-5 justify-center">
						...
						<button
							onClick={() => {
								dispatch(typeAction.setShowModalDelete(true))
								dispatch(typeAction.setDataSelected(id))
							}}
							type="button"
	...
	return (
		<React.Fragment>
			<DeleteModal show={showModalDelete} onHide={() => dispatch(typeAction.setShowModalDelete(false))} onDelete={() => onDelete()} />
