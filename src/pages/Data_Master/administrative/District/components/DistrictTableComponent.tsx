...
import { deleteDistrict, getAllDistrict, getOneDistrict } from "../store/district.asyncAction";
...
const DistrictTableComponent = () => {
	...
	const { getDataLoading, districtList, meta, isUpdated, isDeleted, showModalDelete, dataSelected } = useSelector((state: any) => state?.masterState?.District);
	...
	useEffect(() => {
		...
		dispatch(getAllDistrict(resetMeta));
	...
	useEffect(() => {
		if (isUpdated || isDeleted) {
			...
			dispatch(getAllDistrict(newMeta));
		}
	}, [isUpdated, isDeleted])
	...
	const editData = (id: string) => {
		dispatch(getOneDistrict(id));
	}

	const onDelete = () => {
		dispatch(deleteDistrict(dataSelected));
	}
			...
			cell: ({ row }: any) => {
				...
				return (
					<div className="flex gap-5 justify-center">
						...
						<button
							onClick={() => {
								dispatch(districtAction.setShowModalDelete(true))
								dispatch(districtAction.setDataSelected(id))
							}}
							type="button"
	...
	return (
		<React.Fragment>
			<DeleteModal show={showModalDelete} onHide={() => dispatch(districtAction.setShowModalDelete(false))} onDelete={() => onDelete()} />
			{districtList?.length > 0 && !getDataLoading && (
				<ReactTableComponent
					cols={cols}
					// // columns={columns}
					data={districtList}
