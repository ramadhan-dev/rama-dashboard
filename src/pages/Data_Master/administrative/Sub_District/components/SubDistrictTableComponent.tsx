...
import { deleteSubDistrict, getAllSubDistrict, getOneSubDistrict } from "../store/subdistrict.asyncAction";
...
const SubDistrictTableComponent = () => {

	const dispatch = useDispatch<any>();
	const { getDataLoading, subDistrictList, meta, isUpdated, isDeleted, showModalDelete, dataSelected } = useSelector((state: any) => state?.masterState?.SubDistrict);
	...
	useEffect(() => {
		...
		dispatch(getAllSubDistrict(resetMeta));
	...
	useEffect(() => {
		if (isUpdated || isDeleted) {
			...
			dispatch(getAllSubDistrict(newMeta));
		}
	}, [isUpdated, isDeleted])
	...
	const editData = (id: string) => {
		dispatch(getOneSubDistrict(id));
	}

	const onDelete = () => {
		dispatch(deleteSubDistrict(dataSelected));
	}
	...
	const cols = [
		...
		},
		{
			...
			enableSorting: true,
			cell: ({ row }: any) => {
				return row.original['DistrictName']
			}
		},
		{
			id: "updatedAt",
			...
			cell: ({ row }: any) => {
				...
				return (
					<div className="flex gap-5 justify-center">
						...
						<button
							onClick={() => {
								dispatch(subDistrictAction.setShowModalDelete(true))
								dispatch(subDistrictAction.setDataSelected(id))
							}}
							type="button"
	...
	return (
		<React.Fragment>
			<DeleteModal show={showModalDelete} onHide={() => dispatch(subDistrictAction.setShowModalDelete(false))} onDelete={() => onDelete()} />
			{subDistrictList?.length > 0 && !getDataLoading && (
				<ReactTableComponent
					cols={cols}
					// // columns={columns}
...
export default SubDistrictTableComponent
